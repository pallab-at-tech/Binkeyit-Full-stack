...
const DisplayCartIten = ({ close }) => {

  const { totalPrice, notDiscountTotalPrice , totalQty } = useGlobalContext()
  ...
  return (
    <section className='bg-neutral-900/70 fixed top-0 bottom-0 right-0 left-0 z-50'>
      <div className='bg-white w-full max-w-sm min-h-screen max-h-screen ml-auto'>
        ...
        <div className='max-h-[calc(100vh-150px)] h-full min-h-[75vh] lg:min-h-[80vh] bg-blue-50 p-2 flex flex-col gap-4'>
          {/* display items */}
          {
            cartItem[0] ? (
              <>
...

                  {
                    cartItem[0] && (
                      cartItem.map((item, index) => {
                        return (
                          <div key={item?._id+"CardItemDisplay"} className='flex w-full gap-4'>
...
export default DisplayCartIten
